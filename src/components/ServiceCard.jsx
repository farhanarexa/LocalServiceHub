...
export default function ServiceCard({ service }) {
  // For now, we'll use the service name as the provider since there's no provider field in the DB
  // In the future, you could fetch the user's profile to get their display name
  const provider = service.name.split(' ')[0] || 'Service Provider'; // Fallback to first word of name or generic label
